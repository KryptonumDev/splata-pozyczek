...
export default function CreditTypes({ data: { title, tekst, slider } }) {
    ...
    const transform = (direction) => {
        ...
        //     child.style.animationPlayState = 'running'
        //     child.style.animationDirection = 'unset'
        // }
    }
        ...
        <Wrapper >
            <Container>
                <Content>
                    ...
                    <ControlButtonsWrap>
                        <button onMouseUp={stopTransform} onMouseDown={() => { transform('left') }} className="control left">
                        ...
                        <ControlWrap id='control-wrap'>
                            <Control maxButtonsTransform={maxButtonsTransform} id='control'>
                                {slider.map((el, index) => (
                                    <button className={index === active ? 'active' : ''} onClick={() => { setActive(index) }}><span>{el.tabName}</span></button>
                                ))}
                            </Control>
                        </ControlWrap>
                        <button onMouseUp={stopTransform} onMouseDown={() => { transform('right') }} className="control right">
                            <svg width="16" height="9" viewBox="0 0 16 9" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const transform = (maxButtonsTransform) => keyframes`
    0% {
        transform : translateX(0px) 
    }
    100% {
        transform : translateX(-${maxButtonsTransform}px)
    }
`;
...
const Control = styled.div`
    ...
    gap: 4px;
    width: max-content;
    animation: ${props => transform(props.maxButtonsTransform)} 6s linear;
