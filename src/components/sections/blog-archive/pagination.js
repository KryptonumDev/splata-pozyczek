...
export default function Pagination({ page, posts, setCurrentPage }) {
    ...
    return (
        <Wrapper>
            <Container>
                <Pages>
                    <Left setCurrentPage={setCurrentPage} currentPage={page} />
                    strona
                    {buttons.map((el, index) => {
                        let url = index === 0 ? '/blog/' : '/blog/?page=' + (el + 1)
                        if (page === 1 && (index === page - 1 || index === page || index === page + 1 || index === page + 2 || index === page + 3 || index === page + 4)) {
                            return (
                                <Button to={url} onClick={e => { e.preventDefault(); setCurrentPage(el + 1) }} active={page === el + 1}>
                                    <span>{el + 1}</span>
                                </Button>
                            )
                        }
                    ...
                    <Right setCurrentPage={setCurrentPage} currentPage={page} maxPage={pagesCount} />
                </Pages>
                <InputPagination>
                    ...
                    <Link onClick={e => { e.preventDefault(); setCurrentPage(InputVal) }} to={InputVal === 1 ? '/blog/' : '/blog/?page=' + InputVal} className="dark body1">Przejdź</Link>
                </InputPagination>
            </Container>
        </Wrapper>
    )
...
const Left = ({ setCurrentPage, currentPage }) => (
    <Link className="left" to={currentPage === 1 ? '/blog/' : '/blog/?page=' + (currentPage - 1)} onClick={e => { e.preventDefault(); setCurrentPage(currentPage === 1 ? 1 : currentPage - 1) }}>
...
const Right = ({ setCurrentPage, currentPage, maxPage }) => (
    <Link className="right" to={currentPage === maxPage ? '/blog/?page=' + maxPage : '/blog/?page=' + (currentPage + 1)} onClick={e => { e.preventDefault(); setCurrentPage(currentPage === maxPage ? maxPage : currentPage + 1) }}>
