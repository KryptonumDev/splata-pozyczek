...
export default function Reviews({ data: { title, text }, expert, comments }) {
    ...
    query {
        allWpComment(filter: {comment: {isShown: {eq: true}}}) {
          nodes {
            author {
              node {
                name
                    ...
                    {chosenComments?.map((el, index) => {
                        ...
                        return (
                            <Item key={index} >
                                ...
                                <figure >
                                    <blockquote className="body3" dangerouslySetInnerHTML={{ __html: el.content.rendered }} />
                                    <figcaption className="body2">{el.author_name}</figcaption>
                                </figure >
                            </Item>
                        )
                    })}
