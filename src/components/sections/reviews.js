...
export default function Reviews({ data: { title, text }, expert, comments }) {
    ...
    const { wpPage: { reviews: { reviewsRepeater } } } = useStaticQuery(graphql`
    query {
                    ...
                    {reviewsRepeater?.map((el, index) => {
                        ...
                        return (
                            <Item key={index} >
                                ...
                                <figure >
                                    <blockquote className="body3" dangerouslySetInnerHTML={{ __html: el.text }} />
                                    <figcaption className="body2">{el.author}</figcaption>
                                </figure >
                            </Item>
                        )
                    })}
                </Grid>
                {reviewsRepeater.length < 6
                    ? null
                    : showCount > reviewsRepeater.length
